...
Five ML models compared across Model A (2009-11) and Model B (2005-2024):
  ...
  4. XGBoost                  — best tabular performance, handles vol regimes
...
def train_random_forest_regressor(X_train, y_train, X_test, y_test) -> dict:
    ...
    model.fit(X_train, y_train)
    y_pred = model.predict(X_test)
    fi = {col: float(imp) for col, imp in zip(X_train.columns, model.feature_importances_)}
    result = {
        "name": "RandomForest",
        "cv_r2_scores": cv_scores,
        "cv_r2_mean": float(np.mean(cv_scores)),
...
def train_random_forest_classifier(X_train, y_train, X_test, y_test) -> dict:
    ...
    result = {
        ...
        "feature_importance": fi,
        "fragility_scores": (y_proba * 100).tolist(),
        "y_pred": y_pred.tolist(),
        "y_proba": y_proba.tolist(),
    }
    print(f"  RF Clf test AUC={result['test_metrics'].get('roc_auc')}")
    return result
...
def train_xgboost_regressor(X_train, y_train, X_test, y_test) -> dict:
    ...
    print(f"  XGB Reg test R2={result['test_metrics']['r2']:.4f}")
    return result
...
def train_xgboost_classifier(X_train, y_train, X_test, y_test) -> dict:
    ...
    print(f"  XGB Clf test AUC={result['test_metrics'].get('roc_auc')}")
    return result
...
def train_lstm(X_train, y_train, X_test, y_test,
               lookback: int = 20, epochs: int = 50, mode: str = "regression") -> dict:
    """
    ...
    if mode == "classification":
        ...
        result = {
            ...
            "fragility_scores": (y_raw * 100).tolist(),
            "y_pred": y_pred.tolist(),
        ...
        result = {
            "name": "LSTM",
            "mode": mode,
            "train_metrics": _reg_metrics(y_tr_seq, model.predict(X_tr_seq, verbose=0).flatten()),
            "test_metrics": metrics,
            "y_pred": y_raw.tolist(),
        }
...
def compare_models(results: list[dict], mode: str = "regression") -> list[dict]:
    """
