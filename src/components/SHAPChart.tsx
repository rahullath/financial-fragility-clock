...
function extractShapData(
  ...
  if (regimeComp?.[regime]) {
    return Object.entries(regimeComp[regime])
      .map(([feature, value]) => ({ feature, value: Math.abs(value as number) }))
      .sort((a, b) => b.value - a.value);
  }
...
const SHAPChart: React.FC = () => {
  ...
  const caption = useMemo(() => {
    ...
    return `In the ${regime} regime, ${top.feature} is the dominant driver (${pct}% of total |SHAP|).`;
  }, [data, regime]);

  return (
    <div className="shap-chart" id="chart-shap" aria-label="SHAP feature importance chart">
